...
**A stack span must reach every recording entry point.** Otherwise the caller is forced into a heap array on exactly the path that is supposed to allocate nothing. Two independent rules hold this up:

1. *Recording members must be `readonly`.* A non-`readonly` member of a mutable `ref struct` passes `this` as a writable `ref`-to-`ref struct`, which under "method arguments must match" forces caller-wide safe-context on **every** ref-struct argument in the same invocation — including one passed by `in`, where `scoped` cannot help (it narrows only the ref-safe-context of a by-ref parameter). `readonly` makes `this` a `ref readonly` receiver and the constraint disappears. `End()` and `Dispose()` are the only exceptions on `CommandRecorder`; they mutate `_ended`/`_retired`. This is what lets `BeginRendering(in RenderingInfo)` take a `stackalloc`-backed attachment span (issue #209).
2. *By-value span parameters must be `scoped`.* An un-`scoped` by-value span parameter cannot receive a `stackalloc` — CS9080/CS8350 at the call site (issue #205). `scoped` excludes the parameter from the calculation entirely. Nothing in `Recording/` stores a caller span past the call, so it is always accurate there. `params ReadOnlySpan<T>` is implicitly scoped and needs no modifier.

`tests/Ahjo.Vulkan.Tests/ScopedSpanProbe.cs` is the compile-time guard for both: every entry point is called there with a stack span, so dropping either modifier breaks the build.
