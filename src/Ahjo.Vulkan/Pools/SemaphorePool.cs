...
public sealed unsafe class SemaphorePool : IDisposable
{
    ...
    public void Release(BinarySemaphore sem)
    {
        ...
        if (AhjoValidation.IsEnabled && !_allHandles.Contains((nint)sem.Handle))
            AhjoValidation.Fail("SemaphorePool",
                "Release(BinarySemaphore): handle was not produced by this pool.");
    ...
    public void Release(TimelineSemaphore sem)
    {
        ...
        if (AhjoValidation.IsEnabled && !_allHandles.Contains((nint)sem.Handle))
            AhjoValidation.Fail("SemaphorePool",
                "Release(TimelineSemaphore): handle was not produced by this pool.");
