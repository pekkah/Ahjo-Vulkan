...
internal static unsafe class Program
{
        ...
        try
        {
            while (!window.ShouldClose)
            {
                    ...
                    swap.Extent.width != window.Width || swap.Extent.height != window.Height)
                {
                    ...
                    // Depth rebuild lives on the SUCCESS leg only: doing it on
                    // the minimized leg would allocate a full-size attachment
                    // against the stale extent every poll interval.
                    depth.Dispose();
                ...
                if (acq is AcquireResult.OutOfDate or AcquireResult.Suboptimal)
                {
                    ...
                    // Success leg only -- see the loop top for why.
                    depth.Dispose();
                    depth = DepthBuffer.Create(device, swap.Extent.width, swap.Extent.height);
                    continue;
                }
                ...
                if (pres is AcquireResult.OutOfDate or AcquireResult.Suboptimal)
                {
                    ...
                    // state on the next iteration and sleeps there. One
                    // un-slept iteration is bounded.
                    if (TryRecreate(device, swap, ring, in surface, window))
                    {
                        depth.Dispose();
                        depth = DepthBuffer.Create(device, swap.Extent.width, swap.Extent.height);
    ...
    /// post-loop <c>WaitIdle</c> and the <c>finally</c> that writes the pipeline
    /// cache both still run. Exit code stays 0: a lost surface is an environment
    /// failure, not a defect in the sample.</para>
    /// </remarks>
    ...
    /// an extent test can never see the minimize. The depth attachment is
    /// rebuilt by the caller, on the success leg only.
    /// </remarks>
        ...
        Device device, Swapchain swap, FrameRing ring, in Surface surface, SdlWindow window)
    {
        ...
        catch (VulkanException e) when (e.Result is VkResult.VK_ERROR_SURFACE_LOST_KHR)
        {
            ...
            // of unwinding out of Main with a stack trace. It also keeps the
            // depth rebuild on the success leg, where it belongs. Deliberately
