...
internal static unsafe class Program
{
    ...
    private static int Main(string[] args)
    {
        ...
        while (!window.ShouldClose)
        {
            ...
            // VK_ERROR_SURFACE_LOST_KHR from its own surface query and returns
            // false (#222). DeviceLost is deliberately absent: device loss
            // throws out of acquire, present and Recreate alike, so a sample
            // never observes the state and a branch for it would be
            // unreachable.
            ...
            {
                if (!TryRecreate(device, swap, ring, in surface, window))
                {
            ...
            // Everything left is Timeout, NotReady or SurfaceLost. The first
            // two touch no swapchain state and acquired neither an image nor a
            // signalled semaphore, so retrying next iteration is correct and
            // there is nothing to recycle. SurfaceLost has already moved the
            // swapchain to SwapchainState.SurfaceLost, which the loop-top
            // guard catches on the next iteration.
            ...
            if (pres is AcquireResult.OutOfDate or AcquireResult.Suboptimal)
            {
                ...
                // state on the next iteration and sleeps there. One un-slept
                // iteration is bounded.
                TryRecreate(device, swap, ring, in surface, window);
            }
        ...
        Device device, Swapchain swap, FrameRing ring, in Surface surface, SdlWindow window)
    {
        ...
        catch (VulkanException e) when (e.Result is VkResult.VK_ERROR_SURFACE_LOST_KHR)
        {
            ...
            // of unwinding out of Main with a stack trace. Deliberately narrow:
            // every other VulkanException -- device loss included -- still
            // terminates the sample loudly, because none of them is a state the
            // loop knows how to resume from.
            return false;
        }
