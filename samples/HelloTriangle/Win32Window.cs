...
/// <summary>
/// Minimal visible Win32 window for the sample. Direct user32 / kernel32
/// P/Invokes — no SDL3 or GLFW dependency. The wrapper itself does not
/// link against this; surface creation only needs the <c>HWND</c> +
/// <c>HINSTANCE</c> nints.
/// </summary>
...
internal sealed class Win32Window : IDisposable
{
    ...
    public Win32Window(string title, uint width, uint height)
    {
        Width      = width;
        Height     = height;
        _className = $"AhjoVulkanHelloTriangle_{Guid.NewGuid():N}";
        _wndProc   = WndProc;
        ...
        // Adjust window rect so the *client* area matches the request.
    ...
    /// <summary>
    /// Pump pending Win32 messages once. Sets <see cref="ShouldClose"/> on
    /// <c>WM_CLOSE</c> / <c>WM_QUIT</c> / <c>VK_ESCAPE</c>; sets
    /// <see cref="Resized"/> when the client area changes (consumer
    /// reads + clears).
    /// </summary>
