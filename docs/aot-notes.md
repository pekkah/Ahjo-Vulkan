...
- **Static abstract dispatch via `IChainable<TRoot>` / `IChainRoot`.** The same pattern a second time: `T.SType` and `TRoot.RootSType` are static abstracts read by `ChainBuilder.Push<T>` / `Root()` and, since the properties-chain query, by `PhysicalDevice.TryGetProperties<T>`. The `unmanaged` constraint forbids reference type arguments, so every instantiation is exact — ILC devirtualizes it with no reflection and no runtime type lookup.
...
- **Gated device-extension entry points are `delegate* unmanaged[Stdcall]` fields, not delegates.** `DeviceFunctionTable` resolves every extension command through `vkGetDeviceProcAddr` into a plain function-pointer field — the `VK_EXT_mesh_shader` trio (#201) and the seven `VK_KHR_acceleration_structure` pointers (#202), four of which are device-level create/destroy/query rather than `vkCmd*`. `AccelerationStructure` additionally stores one such pointer (`vkDestroyAccelerationStructureKHR`) inline on the handle struct so it can destroy an extension object from `Dispose` without holding a managed `Device`. All of it is compile-time-typed unmanaged dispatch: no delegate marshalling, no `Marshal.GetDelegateForFunctionPointer`, and no new trim surface — adding an extension group adds fields, never reflection.
