...
//       }
//   - For each leaf struct L that has an sType field but is neither a root
//     nor a chainable (e.g. VkCommandBufferAllocateInfo, VkMemoryBarrier):
//       partial struct L
//       {
//           public L() { sType = VK_STRUCTURE_TYPE_L; }
//       }
//
// The parameterless ctor makes `new VkX { ... }` object-initializer syntax
// write a valid sType (issues #94, #95). Four structs are both roots and
...
// Leaves are mutually exclusive with both — separate file, no marker
// interface, just the ctor.
//
...
internal static class Program
{
    private static int Main(string[] args)
    {
        ...
        // Leaves: sType-bearing structs that participate in no pNext chain
        // (neither structextends source nor target). Same auto-init story as
        // chain participants, no marker interface — these are stand-alone
        // sType-typed parameter records (issue #95).
        var chainableStructNames = new HashSet<string>(
            chainables.Select(c => c.StructName), StringComparer.Ordinal);
        var emittedLeaves = 0;
        var skippedLeafForMissingSType = 0;
        var skippedLeafForMissingStruct = 0;
        foreach (var (structName, sType) in structToSType.OrderBy(kv => kv.Key, StringComparer.Ordinal))
        {
            if (chainableStructNames.Contains(structName)) continue;
            if (emittedRoots.Contains(structName)) continue;

            if (!availableSTypes.Contains(sType))
            {
                skippedLeafForMissingSType++;
                continue;
            }
            if (!availableStructs.Contains(structName))
            {
                skippedLeafForMissingStruct++;
                continue;
            }

            EmitSelf(outputDir, structName, sType);
            emittedLeaves++;
        }

        Console.WriteLine($"Emitted {emittedExtenders} IChainable partials, {emittedRoots.Count} IChainRoot partials, and {emittedLeaves} leaf-init partials.");
        ...
        Console.WriteLine($"Leaf skip detail: {skippedLeafForMissingSType} (sType absent), {skippedLeafForMissingStruct} (struct body absent).");
        return 0;
    }
    ...
    private static void EmitSelf(string outputDir, string structName, string sType)
    {
        var sb = new StringBuilder();
        sb.AppendLine("// <auto-generated />");
        sb.AppendLine("// Source: vk.xml struct sType (no structextends relationship).");
        sb.AppendLine("// Edits to this file are overwritten by RegenerateChains.");
        sb.AppendLine("namespace Ahjo.Vulkan.Native;");
        sb.AppendLine();
        sb.Append("public unsafe partial struct ").AppendLine(structName);
        sb.AppendLine("{");
        ...
        File.WriteAllText(Path.Combine(outputDir, structName + ".Self.g.cs"), sb.ToString());
    }
